...
The network worker owns MQTT, DTF2 reassembly, record validation, duplicate
handling, and DTA1 publication.  The Qt thread only requests immutable Mission
snapshots, so a slow map refresh can never delay acknowledgements.
"""
...
class LiveTelemetrySource:
    ...
    def __init__(self, config: LiveReceiverConfig):
        ...
        self._reassembler = TelemetryReassembler(
            timeout_seconds=30.0, max_inflight=config.max_inflight)
    ...
    def _enqueue_publication(self, topic: str, payload: bytes, qos: int) -> None:
        ...
        publication = _InboundPublication(topic, bytes(payload), qos, now)
        try:
        ...
        try:
            ...
            for fragment in fragments:
                ...
                if cached is not None:
                    ...
                    continue
                completed = self._reassembler.push(fragment)
