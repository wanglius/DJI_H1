...
class BaiduMissionMap(QWebEngineView):
    ...
    def __init__(self, parent=None, *, ak: str):
        super().__init__(parent)
        self.setPage(_MapPage(self))
        self._ak = ak
        self._ready = False
    ...
    def _payload(model: MissionMapModel) -> str:
        ...
        # Localize this private web-view projection, never the shared API model.
        for event in payload["events"]:
            event["event_label"] = event_name(event["event_name"])
    ...
    def _send_payload(self, payload: str, fit: bool) -> None:
        ...
        self.page().runJavaScript(
            f"(() => {{ try {{ window.setMissionData({payload}, {fit_text}); "
            "return true; } catch (_) { return false; } })();",
            lambda ok: self.map_error.emit("Map render failed") if ok is not True else None)
