...
class LiveViewerTests(unittest.TestCase):
    ...
    def test_one_window_renders_a_live_snapshot(self) -> None:
        ...
        with patch("dji_h1_ground.ui.MissionMapPanel", MapStub):
        ...
        try:
            ...
            self.assertIn("待定位", viewer.events_list.item(1).text())
            self.assertIn("反射率计算 9",
                          viewer.spectrum_info.text())
            self.assertIn("实时链路", viewer.flight_info.text())
        finally:
            viewer.close()
            app.processEvents()
